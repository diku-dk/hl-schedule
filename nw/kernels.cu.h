...
__global__ void
initInputKer(int* input_d, const int N, const int penalty) {
    int gtid = blockIdx.x * BLOCK_SIZE + threadIdx.x + 1;

    input_d[gtid*N] = -gtid * penalty;
    input_d[gtid]   = -gtid * penalty;    
} 
			  ...
			  int block_width) 
{
  ...
  uint64_t b_index_x = bx;
  uint64_t b_index_y = i - 1 - bx;

  uint64_t cols = cols0; 
  uint64_t index   = cols * BLOCK_SIZE * b_index_y + BLOCK_SIZE * b_index_x + tx + ( cols + 1 );
  uint64_t index_n   = cols * BLOCK_SIZE * b_index_y + BLOCK_SIZE * b_index_x + tx + ( 1 );
  uint64_t index_w   = cols * BLOCK_SIZE * b_index_y + BLOCK_SIZE * b_index_x + ( cols );
  uint64_t index_nw =  cols * BLOCK_SIZE * b_index_y + BLOCK_SIZE * b_index_x;
  ...
  for( int m = 0 ; m < BLOCK_SIZE ; m++){
      if ( tx <= m ){
          ...
          temp[t_index_y][t_index_x] =
            max3( temp[t_index_y-1][t_index_x-1] + ref[t_index_y-1][t_index_x-1]
			  ...
			  int block_width) 
{
  ...
  uint64_t b_index_x = bx + block_width - i  ;
  uint64_t b_index_y = block_width - bx -1;
  uint64_t cols = cols0; 
